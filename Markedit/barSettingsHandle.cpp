...
void MainWindow::showSettingsDialog()
{
    ...
    layout->addWidget(new QLabel(tr("语言:")));
    ...
    if (dialog.exec() == QDialog::Accepted) {
        QFont font = font_combo->currentFont();
        font.setPointSize(size_spin->value());
        _applyFontSettings(font);

        QString lang_code = lang_combo->currentData().toString();
...
void MainWindow::_switchLanguage(const QString &lang_code)
{
    ...
    if (m_translator.load(qmPath)) {
        ...
        QMessageBox::warning(this, tr("错误"), tr("无法加载语言文件: ") + qmPath);
    }
}
